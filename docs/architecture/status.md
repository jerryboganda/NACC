...
Per the user's compute directive, heavy builds/tests/packaging now belong on GitHub Actions: `.github/workflows/ci.yml` runs fmt, frontend build+vitest, bindings-freshness, and full clippy+test batteries under BOTH MSVC and the pinned GNU toolchain on `windows-latest` runners; `desktop-build.yml` produces the unsigned NSIS bundle on demand. The rust-msvc job is the first MSVC verification this project has ever had — the local machine cannot link MSVC, so its result is new evidence, not a re-statement. **Verified on Actions (run 35260972351, 2026-09-17): all five jobs green** — fmt 15s, frontend build+vitest 14s, bindings freshness 1m58s, GNU clippy+tests 8m27s, **MSVC clippy+tests 6m8s** — the entire workspace (23 crates, including the Job-Object process-tree tests) now compiles, lints clean at `-D warnings`, and passes every test under MSVC, closing the long-standing "MSVC unverified" gap. The first run also caught a real bug the local battery had missed (a `RoleProfile` test literal without the V7 fields, in a crate whose tests were not re-run after the schema change) plus a per-toolchain clippy-component lesson; both fixed in `8a9fa9a`. Local background GNU builds remain only for pre-push iteration (strict technical requirement); the production VPS is out of scope for compute entirely. CI runs require commits on origin — pushing `main` for CI is authorized by that directive; releases/signing still require explicit human sign-off (docs/RELEASE-CHECKLIST.md).
...
Fixed while wiring: `RunSnapshotView.pending_approval_count` was `usize`, which specta refuses to export (BigInt precision guard) — now `u32`, matching `attempts`/`sequence`; and the never-called `ProviderNodeExecutor::with_timeout` builder was removed rather than `allow`ed dead (node-declared timeouts remain Task D remainder). Trap for the next agent: tauri-specta exports *sync* Rust commands without the `{status:"ok"|"error"}` wrapper — `listWorkflowTemplates` resolves to the bare array and rejects on transport failure, while the eight async commands wrap as usual.

Verified on the final code state: GNU `cargo test -p nacc-app` **15 passed / 0 failed** (was 6; +3 workflows, +6 routing); GNU `clippy -p nacc-app --all-targets -- -D warnings` clean; `cargo fmt --all` applied; `cargo run -p nacc-app -- --export-bindings` regenerates `src/bindings.ts` with all nine commands present; frontend `npm run build` passes and `npm test` **26/26** (App 4, Providers 4, RoleMatrix 7, RunConsole 11). NOT demonstrated: no live desktop end-to-end run, no real agent-CLI launch from the GUI, storage/adapter suites not re-run this session (those crates are untouched by this slice), MSVC still unverified locally. No push, no CI dispatch, no provider authentication was performed.
