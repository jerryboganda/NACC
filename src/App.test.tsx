...
vi.mock("./bindings", () => ({
  commands: {
    ...
    pauseWorkflowRun: async () => ({ status: "ok", data: null }),
    cancelWorkflowRun: async () => ({ status: "ok", data: null }),
    ...
    getGithubAuthStatus: async () => ({ status: "ok", data: { authenticated: false, login: null } }),
    getGithubRepository: async () => ({ status: "error", error: "not loaded" }),
    listGithubBranches: async () => ({ status: "ok", data: [] }),
    listGithubPullRequests: async () => ({ status: "ok", data: [] }),
    listGithubCheckRuns: async () => ({ status: "ok", data: [] }),
    listGithubWorkflowRuns: async () => ({ status: "ok", data: [] }),
    listGithubWorkflowJobs: async () => ({ status: "ok", data: [] }),
    getGithubFailedRunEvidence: async () => ({ status: "ok", data: [] }),
    listGithubArtifacts: async () => ({ status: "ok", data: [] }),
    listGithubEnvironments: async () => ({ status: "ok", data: [] }),
    listGithubPendingDeployments: async () => ({ status: "ok", data: [] }),
    rerunFailedGithubWorkflow: async () => ({ status: "ok", data: null }),
    getWorkflowTemplate: async () => ({ status: "ok", data: null }),
    listWorktreeLeases: async () => ({ status: "ok", data: [] }),
    listAuditRecords: async () => ({ status: "ok", data: [] }),
    listQualityGateResults: async () => ({ status: "ok", data: [] }),
    listReviewFindings: async () => ({ status: "ok", data: [] }),
  },
}));
...
describe("App", () => {
  ...
  it("shows a loading state before the command resolves", async () => {
    ...
    expect(await screen.findByText("No saved provider detections.")).toBeInTheDocument();
